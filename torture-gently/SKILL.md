...
elif dm=$(mode "$d") && fm=$(mode "$f") &&
  [ $(( ($dm & 010 && $fm & 040) || ($dm & 001 && $fm & 004) )) -eq 0 ]; then echo present
else echo wide; fi
```
...
On POSIX the key is `wide` only when the directory lets a group or other account traverse it and the file lets that same class read it; either one alone does not expose the key. Windows grants traverse bypass to every account by default, so only the file's own ACL decides.
