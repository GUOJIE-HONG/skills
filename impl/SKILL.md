...
1. Read the spec and tickets, then `design.md`, asking the user for its path if they have not given it. It records the chosen implementation direction and the conventions to follow, each with an evidence path. Treat its decisions as settled, and raise any that the code contradicts with the user.
