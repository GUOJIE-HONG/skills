...
- What remains ready or blocked, with unfinished prerequisites; say all done only when every in-scope task and ticket is complete.
- Retained worktree paths, branches, and checkpoints needed to resume.

Remove only run-created worktrees and branches whose work is integrated, validated, and clean. Preserve failed, unintegrated, or dirty work; never force cleanup or remove pre-existing worktrees.
